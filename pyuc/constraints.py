...
def cnt_supply_eq_demand(sets, data, var, constraints={}):
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            label = f"supply_eq_demand_(s={s},i={i})"

            condition = (
                total_power_generated[(s, i)]
                + var["unserved_power"].var[(s, i)]
                ==
                data["demand"]["Demand"][(s, i)]
                + total_power_charged[(s, i)]
                )
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for r in sets["reserves"].indices:
                label = f"reserve_enabled_exceeds_reserve_requirement_(s={s},i={i},r={r})"

                condition = (
                    total_reserve_enabled[(s, i, r)]
                    + var["unserved_reserve"].var[(s, i, r)]
                    >=
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units"].indices:
                label = f"power_lt_capacity_(s={s},i={i},u={u})"
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units_commit"].indices:
                label = f"power_lt_committed_capacity_(s={s},i={i},u={u})"

                if u in sets["units_reserve"].indices:
                    var_raise_reserves_enabled = \
                        sum(var["reserve_enabled"].var[(s, i, u, r)] for r in sets["raise_reserves"].indices)
                else:
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units_reserve"].indices:
                for r in sets["reserves"].indices:
                    label = f"reserve_enabled_lt_reserve_capability_(s={s},i={i},u={u},r={r})"

                    condition = (
                        var["reserve_enabled"].var[(s, i, u, r)]
                        <=
                        var["num_committed"].var[(i, u)]
...
def cnt_power_gt_minimum_generation(sets, data, var, constraints={}):
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units_commit"].indices:
                label = f"power_gt_minimum_generation_(s={s},i={i},u={u})"

                if u in sets["units_reserve"].indices:
                    var_lower_reserves_enabled = \
                        sum(var["reserve_enabled"].var[(s, i, u, r)] for r in sets["lower_reserves"].indices)
                else:
...
def cnt_ramp_rate_up(sets, data, var, constraints={}):
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units_commit"].indices:
                label = f"ramp_rate_up_(s={s},i={i},u={u})"
...
def cnt_ramp_rate_down(sets, data, var, constraints={}):
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            ...
            for u in sets["units_commit"].indices:
                label = f"ramp_rate_down_(s={s},i={i},u={u})"
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units_variable"].indices:
                label = f"variable_resource_availability(s={s},i={i},u={u})"

                condition = \
                    var["power_generated"].var[(s, i, u)] \
                    <= \
                    data["variable_traces"][technology_type[u]][(s, i)] \
                    * data["units"]["NumUnits"][u] \
                    * data["units"]["CapacityMW"][u]
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units_storage"].indices:
                label = f"charge_lt_rt_loss_adjusted_capacity(s={s},i={i},u={u})"

                condition = \
                    var["power_charged"].var[(s, i, u)] \
                    <= \
                    data["units"]["NumUnits"][u] \
                    * data["units"]["CapacityMW"][u] \
...
def cnt_storage_energy_continuity(sets, data, var, constraints={}):
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices[1:]:
            for u in sets["units_storage"].indices:
                label = f"storage_energy_continuity(s={s},i={i},u={u})"

                condition = \
                    var["stored_energy"].var[(s, i-1, u)] \
                    - var["stored_energy"].var[(s, i, u)] \
...
def cnt_stored_energy_lt_storage_capacity(sets, data, var, constraints={}):
    ...
    for s in sets["scenarios"].indices:
        for i in sets["intervals"].indices:
            for u in sets["units_storage"].indices:
                label = f"stored_energy_lt_storage_capacity(s={s},i={i},u={u})"
...
def total_power_generated_in_interval(sets, power_generated):
    ...
    scenarios = sets["scenarios"].indices
    units = sets["units"].indices
    power_generated = power_generated.var

    return {(s, i): pp.lpSum(
            [power_generated[(s, i, u)] for u in units]
        ) for s in scenarios for i in intervals}
...
def total_reserve_enabled_in_interval(sets, reserve_enabled):
    ...
    return {(s, i, r): pp.lpSum([reserve_enabled[(s, i, u, r)] for u in units_reserve])
            for s in scenarios for i in intervals for r in reserves}
...
def total_power_charged_in_interval(sets, data, power_charged):
    ...
    charge_dict = {
        (s, i): pp.lpSum([
            (1 / data["units"]["RoundTripEfficiencyFrac"][u]) * power_charged[(s, i, u)]
            for u in units_storage]) for s in scenarios for i in intervals
    }
...
def ramp_calculator(sets, data, var):
    ...
    for s in sets["scenarios"].indices:
        for u in sets["units"].indices:
            ...
            rampMW[(s, first_interval, u)] = \
                var["power_generated"].var[(s, first_interval, u)] - units_initial_power

            for i in sets["intervals"].indices[1:]:
                rampMW[(s, i, u)] = \
                    var["power_generated"].var[(s, i, u)] \
                    - var["power_generated"].var[(s, i-1, u)]
